...
class Generator(Module):
    def __init__(self,
        ...
        self.latent = Sequential(Linear(latent_dim, 32),
                                 LeakyReLU(0.2, inplace=True))

        self.label_conditioned_generator = Linear(label_dim, 32)

        self.model = Sequential(
            ConvTranspose3d(in_channels=1, out_channels=2, kernel_size=(2, 2, 2), stride=2, bias=False,
                            output_padding=(1, 1, 1)),
            BatchNorm3d(2, momentum=0.1, eps=0.8),
            ReLU(True),
            ConvTranspose3d(in_channels=2, out_channels=4, kernel_size=(3, 3, 3), stride=2, bias=False, output_padding=(1, 1, 1)),
            BatchNorm3d(4, momentum=0.1, eps=0.8),
            ReLU(True),
            ConvTranspose3d(in_channels=4, out_channels=8, kernel_size=(3, 3, 3), stride=1, bias=False),
            BatchNorm3d(8, momentum=0.1, eps=0.8),
            ReLU(True),
            ConvTranspose3d(in_channels=8, out_channels=4, kernel_size=(3, 3, 3), stride=1, bias=False, dilation=2),
            BatchNorm3d(4, momentum=0.1, eps=0.8),
            ReLU(True),
            ConvTranspose3d(in_channels=4, out_channels=1, kernel_size=(3, 3, 3), stride=1, bias=False, dilation=2),
    ...
    def forward(self,
        ...
        latent_output = latent_output.view(-1, 1, 2, 4, 4)

        target_output = self.label_conditioned_generator(target.view(-1, 1))
        target_output = target_output.view(-1, 1, 2, 4, 4)
